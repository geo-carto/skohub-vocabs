...
function nodeRadius(direction, ringDepth = 0) {
  const base =
    direction === "center"
      ? 34
      : direction === "broader" || direction === "narrower"
      ? 28
      : 22
...
function buildFlatMap(vocabId, data, lang) {
  ...
  function visit(concept) {
    ...
    map.set(concept.id, {
      id: concept.id,
      label: lbl(concept),
      narrowerIds: idsOf(concept.narrower),
      broaderIds: [],
      _related: idsOf(concept.related),
      _exactMatch: idsOf(concept.exactMatch),
      _closeMatch: idsOf(concept.closeMatch),
      _relatedMatch: idsOf(concept.relatedMatch),
      _broadMatch: idsOf(concept.broadMatch),
      _narrowMatch: idsOf(concept.narrowMatch),
    })
    for (const child of Array.isArray(concept.narrower)
      ? concept.narrower
      : concept.narrower
      ? [concept.narrower]
      : [])
  ...
  map.set(vocabId, {
    id: vocabId,
    label: data.title?.[lang] || data.title?.es || data.title?.en || vocabId,
    narrowerIds: idsOf(data.hasTopConcept),
    broaderIds: [],
    _related: [],
    _exactMatch: [],
    _closeMatch: [],
  ...
  for (const [, node] of map) {
    for (const cid of node.narrowerIds) {
      ...
      if (child && !child.broaderIds.includes(node.id))
        child.broaderIds.push(node.id)
    }
  }
  return map
}
...
function computeMaxDepths(focalId, allMap) {
  ...
  while (qB.length) {
    ...
    for (const bid of allMap.get(id)?.broaderIds || []) {
      if (!vB.has(bid) && allMap.has(bid)) {
        vB.add(bid)
        qB.push({ id: bid, d: d + 1 })
      }
    }
  }
  let maxNarrow = 0
  ...
  while (qN.length) {
    ...
    for (const nid of allMap.get(id)?.narrowerIds || []) {
      if (!vN.has(nid) && allMap.has(nid)) {
        vN.add(nid)
        qN.push({ id: nid, d: d + 1 })
      }
    }
  }
...
function layoutNodes(nodes, edges, cx, cy) {
  ...
  function place(id, aLo, aHi, depth) {
    ...
    if (!ch.length) return
    const R = RING_RADII[Math.min(depth, RING_RADII.length - 1)] || depth * 110
  ...
  nodes
    ...
    .forEach((n, i, arr) => {
      ...
      const R = RING_RADII[Math.min(n.ringDepth, RING_RADII.length - 1)]
      ...
      {Array.from({ length: max + 1 }, (_, d) => d).map((d) => (
        <button
          ...
          style={{
            ...
            fontFamily: "inherit",
            border: `1.5px solid ${
              value === d ? "rgb(196,95,40)" : "rgb(220,205,185)"
            }`,
...
const ConceptEgoModal = ({ concept, language, customDomain, onClose }) => {
  ...
  const cx = w / 2,
    cy = h / 2
  const basePos = layoutNodes(nodes, edges, cx, cy)
  ...
  const _idsOf = (arr) =>
    !arr
      ...
      : (Array.isArray(arr) ? arr : [arr])
          .map((x) => (typeof x === "string" ? x : x?.id))
          .filter(Boolean)
  const matchCount = SEMANTIC_PROPS.reduce(
    (n, p) => n + _idsOf(concept[p]).length,
    0
  )
        ...
        >
          ...
          >
            <DepthBtns
              label="↑ Broader:"
              value={depthBroader}
              max={maxDepths.maxBroader}
              onChange={setDepthBroader}
            />
            <DepthBtns
              label="↓ Narrower:"
              value={depthNarrow}
              max={maxDepths.maxNarrow}
              onChange={setDepthNarrow}
            />
            <button
              ...
              style={{
                ...
                fontSize: 11,
                border: `1.5px solid ${
                  showMatches ? "rgb(196,95,40)" : "rgb(220,205,185)"
                }`,
                ...
                {nodes.map((n) => {
                  ...
                  return (
                    <g
                      ...
                      onMouseDown={(e) =>
                        onNodeMouseDown(e, n.id, nodeUrl, pos)
                      }
                    >
                      {isCenter && (
                        <circle
            ...
            {[
              {
                label: "+",
                fn: () => {
                  const s = Math.min(4, scaleRef.current + 0.2)
                  scaleRef.current = s
                  setScale(s)
                },
              },
              {
                label: "−",
                fn: () => {
                  const s = Math.max(0.15, scaleRef.current - 0.2)
                  scaleRef.current = s
                  setScale(s)
                },
              },
